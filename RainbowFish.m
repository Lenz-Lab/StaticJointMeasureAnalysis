function RainbowFish(Bone,NodalIndex,AllNodalData,CLimits,ColorMap_Flip,ROI,views)
...
if isempty(ROI) == 0
    for n = 1:length(ROI(:,1))
        ...
        if ColorMap_Flip == 1
            if AllNodalData(n,3) >=  0 && AllNodalData(n,3) <= 4
            ...
            if AllNodalData(n,3) > 4
                patch(PR,'FaceColor', [0 0 1], ...
                    'EdgeColor','none',...
                    'FaceLighting','gouraud',...
                    'AmbientStrength', 0.15,'facealpha',1);
                material('dull');
